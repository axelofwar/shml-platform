/**
 * FusionAuth Lambda: GitHub Registration - Assign Default Role
 *
 * PURPOSE: Automatically assign 'viewer' role to users registering via GitHub OAuth
 *
 * TRIGGER: GitHub (ExternalJWT) Identity Provider → User Reconcile Lambda
 *
 * PROBLEM: When users sign in with GitHub, FusionAuth creates a registration
 ...
 *
 * SOLUTION: This lambda runs during GitHub sign-in and adds the viewer role
 * to the user's OAuth2-Proxy registration before the JWT is issued.
 *
 * CONFIGURATION:
 * 1. Go to FusionAuth Admin → Settings → Lambdas → Add Lambda
 * 2. Name: "GitHub Registration - Assign Default Role"
 * 3. Type: "ExternalJWT Reconcile" (for GitHub ExternalJWT provider)
 * 4. Paste this code
 * 5. Go to Settings → Identity Providers → GitHub
 ...
 * @param {Object} jwt - The GitHub JWT/token claims
 */
...
function reconcile(user, registration, jwt) {
  ...
  if (registration.applicationId === oauthProxyAppId) {
    ...
    if (registration.roles.length === 0) {
      ...
      console.info('Assigned default role "' + defaultRole + '" to GitHub user ' + user.email + ' for OAuth2-Proxy registration');
    }
  }
}
