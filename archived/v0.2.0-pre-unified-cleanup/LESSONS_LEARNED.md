...
## 🌐 Critical Pattern #5: Ray Jobs API via Traefik (Not Port 8265)

**Added:** 2025-11-25  
**Issue:** Remote job submission fails with "connection refused"

### The Problem

```bash
# ❌ WRONG - Port 8265 is NOT exposed externally
curl http://<TAILSCALE_IP>:8265/api/jobs/  # Connection refused
ray job submit --address http://<TAILSCALE_IP>:8265 ...  # Fails
```

**Result:** Cannot submit Ray jobs remotely; Ray JobSubmissionClient fails.

### Why This Happens

- Ray Dashboard is exposed via Traefik at `/ray/`
- But port 8265 (Ray Jobs API native port) is NOT exposed in Docker
- Docker only exposes ports explicitly mapped in docker-compose.yml
- Traefik proxies the full Ray Dashboard including `/api/` endpoints

### The Solution

Use the Traefik-proxied Ray Jobs API at `/ray/api/jobs/`:

```bash
# ✅ CORRECT - Use Traefik-proxied endpoint
curl http://<TAILSCALE_IP>/ray/api/jobs/  # Works!

# Submit job via REST API
curl -X POST http://<TAILSCALE_IP>/ray/api/jobs/ \
  -H "Content-Type: application/json" \
  -d '{"entrypoint": "echo hello", "submission_id": "my-job-123"}'

# Check job status
curl http://<TAILSCALE_IP>/ray/api/jobs/my-job-123

# Get job logs
curl http://<TAILSCALE_IP>/ray/api/jobs/my-job-123/logs
```
...
job_data = {
    ...
    "metadata": {"project": "pii-pro"}
}

r = requests.post(f"{RAY_URL}/api/jobs/", json=job_data, timeout=10)
print(f"Submitted: {r.json()}")  # {'job_id': 'my-job-xxx', 'submission_id': 'my-job-xxx'}

# Check status
time.sleep(2)
r = requests.get(f"{RAY_URL}/api/jobs/{job_id}", timeout=5)
print(f"Status: {r.json().get('status')}")  # PENDING, RUNNING, SUCCEEDED, FAILED
```

### ⚠️ Known Issue: GPU Access in Job Containers

Even with successful job submission, **job containers don't have GPU access**:

```python
# Job output shows:
# PyTorch: 2.1.0+cu118
# CUDA available: False  ← GPU not accessible
# No GPU detected in this container
```

**Root Cause:** Ray spawns job containers without GPU passthrough.  
**Fix Required:** Update docker-compose.yml on server to enable GPU for Ray job containers.

### Verification

```bash
# Test from macOS Docker container (Tailscale accessible)
docker run --rm python:3.11-slim python -c "
import requests
r = requests.get('http://<TAILSCALE_IP>/ray/api/version', timeout=5)
print(r.json())  # {'version': '4', 'ray_version': '2.9.0', ...}
"
```

### Where This Is Documented

- **REMOTE_QUICK_REFERENCE.md**: Updated Ray Jobs API section
- **pii-pro/docs/TRAINING.md**: "Ray Cluster Training" section
- **pii-pro/docs/internal/V7_REFACTOR_PROGRESS.md**: Ray cluster validation results
...
6. **Ray Jobs via Traefik**: Port 8265 not exposed; use `/ray/api/jobs/` endpoint
7. **Document In-Line**: Future you (or teammates) will thank you
8. **Test Cold Starts**: Stop everything and start fresh to catch issues
