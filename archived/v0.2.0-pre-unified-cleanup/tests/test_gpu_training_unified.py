        ...
        def update(self, n=1):
            if not self.disable:
                ...
                if self.total > 0:
                    ...
                    print(
                        f"\r{self.desc}: {self.n}/{self.total} ({pct:.0f}%)",
                        end="",
                        flush=True,
                    )
        ...
        try:
            if self.use_ray_api:
                ...
                response = requests.get(
                    f"{self.server_url.replace(':8266', ':8265')}/api/version"
                )
...
"""
            ...
            print(
                f"  Memory: {resources['cluster_total'].get('memory_bytes', 0) / 1e9:.1f} GB"
            )
...
"""
    ...
    def test_yolov8_training(self, epochs: int = 2, num_images: int = 50) -> bool:
        ...
        self.print_header(
            f"Test 7-8: YOLOv8m-face Training ({epochs} epochs) + Inference ({num_images} images)"
        )
...
"""
    ...
    def _run_gpu_allocation_test(self, name: str, code: str, gpu: float) -> bool:
        ...
        try:
            ...
            job_id = self._submit_job(
                name=name, code=code, cpu=4, memory_gb=8, gpu=gpu, timeout_minutes=10
            )
        ...
        try:
            if self.use_ray_api:
                ...
                response = requests.post(
                    f"{self.server_url}/jobs/submit", json=job_data, timeout=30
                )
    ...
    def _wait_for_job(
        ...
        try:
            while time.time() - start_time < timeout:
                try:
                    if self.use_ray_api:
                        ...
                        response = requests.get(
                            f"{self.server_url}/jobs/{job_id}", timeout=10
                        )
                        if response.status_code == 200:
                    ...
                    if status_str in ["SUCCEEDED", "FAILED", "STOPPED"]:
                        ...
                        if self.use_ray_api:
                            ...
                            log_response = requests.get(
                                f"{self.server_url}/jobs/{job_id}/logs", timeout=10
                            )
                            logs = (
                                log_response.json().get("logs", "")
            ...
            return (
                "TIMEOUT",
                f"Job monitoring timed out after {timeout}s. Last status: {last_status}",
            )
...
Examples:
  ...
  python test_gpu_training_unified.py --local --all
    ...
    parser.add_argument(
        "--server", type=str, help="Server URL (e.g., http://100.80.251.28:8266)"
    )
    parser.add_argument(
        "--use-ray-api", action="store_true", help="Use Ray Job Submission API directly"
    )

    # Test options
    parser.add_argument("--quick", action="store_true", help="Quick smoke test only")
    parser.add_argument(
        "--full-training", action="store_true", help="Run full YOLOv8 training test"
    )
    parser.add_argument(
        "--gpu-mode",
        choices=["single", "multi", "fractional", "all"],
        default="all",
        help="GPU allocation mode to test",
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Run all tests (equivalent to --full-training --gpu-mode all)",
    )
    ...
    if args.server:
        ...
        server_url = "http://localhost:8266"
        print(
            f"{Colors.YELLOW}No connection mode specified, using localhost{Colors.NC}"
        )
    ...
    results = tester.run_all_tests(
        quick_mode=args.quick, full_training=args.full_training, gpu_mode=args.gpu_mode
    )
