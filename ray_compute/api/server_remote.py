...
from fastapi import FastAPI, HTTPException, UploadFile, File, Form, BackgroundTasks, Security, Depends
...
class JobSubmission(BaseModel):
    ...
    return_artifacts: bool = Field(default=True, description="Return artifacts to client")
    cleanup_after: bool = Field(default=True, description="Delete artifacts after retrieval")
...
"""
        ...
        if job.cleanup_after:
            background_tasks.add_task(
                schedule_cleanup,
                job_id,
                delay_hours=ARTIFACT_RETENTION_HOURS
            )
        
        logger.info(f"Job submitted: {job_id} (Ray: {ray_job_id})")
        
        return JobInfo(**{k: v for k, v in job_info.items() if k != "ray_job_id" and k != "cleanup_after"})
...
async def list_jobs(
    ...
    try:
        ...
        for job_id, job_data in job_store.items():
            ...
            jobs.append(JobInfo(**{k: v for k, v in job_data.items() if k not in ["ray_job_id", "cleanup_after"]}))
...
async def get_job(job_id: str):
    ...
    try:
        ...
        if ray_job_id:
            ...
            if info.start_time:
                job_data["started_at"] = datetime.fromtimestamp(info.start_time / 1000).isoformat()
            if info.end_time:
                job_data["ended_at"] = datetime.fromtimestamp(info.end_time / 1000).isoformat()
            if info.message:
    ...
    return JobInfo(**{k: v for k, v in job_data.items() if k not in ["ray_job_id", "cleanup_after"]})
...
async def download_job_artifacts(job_id: str, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create artifact archive: {str(e)}")
