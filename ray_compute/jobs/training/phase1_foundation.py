...
try:
    from libs.training.trajectory_filter import TrajectorySegmentFilter

    TSF_AVAILABLE = True
except ImportError:
    TSF_AVAILABLE = False
    print("⚠ TrajectorySegmentFilter not found in libs.training.trajectory_filter")
...
if _device_to_yield == 0:
    _job_id = os.environ.get("RAY_JOB_ID", f"phase1-foundation-{os.getpid()}")
...
class SAPOOptimizer:
    ...
    def update_history(
        ...
        """
        Update SAPO history with epoch results.

        This method is called after each training phase/epoch to update
        the optimizer's internal state for adaptive learning rate adjustments.

        Args:
            epoch: Current epoch number
            loss: Training loss for the epoch
            metrics: Dictionary with mAP50, recall, precision metrics
        """
        # Update loss tracking
        self.update_loss(loss)

        # Store metrics history for preference tracking
    ...
    def get_adaptive_lr(self) -> float:
        ...
        Returns:
            Current learning rate after SAPO adaptation
        """
        return self.current_lr
    ...
    def handle_stage_transition(
        self,
        current_lr: float,
        new_stage_target: Optional[Dict[str, float]] = None,
    ) -> float:
        """
        Handle curriculum stage transition with SAPO.

        Args:
            current_lr: Current learning rate before transition
            new_stage_target: Target metrics for the new stage (optional)

        Returns:
            Adjusted learning rate for the new stage
        """
        self.stage_transitions += 1
        self.stage_baseline_loss = self.loss_ema

        # Store current state as preference baseline
        if hasattr(self, "metrics_history") and self.metrics_history:
            last_metrics = self.metrics_history[-1]["metrics"]
            stage_name = f"stage_{self.stage_transitions}"
            self.preference_weights[stage_name] = {
                "mAP50": last_metrics.get("mAP50", 0),
                "recall": last_metrics.get("recall", 0),
                "weight": 1.0 - (0.1 * self.stage_transitions),
            }

        # Compute new learning rate for stage transition
        # Use conservative LR reduction for stage transitions
        transition_factor = 0.8**self.stage_transitions
        new_lr = current_lr * transition_factor
        ...
        print(f"  📊 SAPO: Stage transition #{self.stage_transitions}")
        print(f"     Previous LR: {current_lr:.6f} → New LR: {new_lr:.6f}")
        if self.loss_ema:
            print(f"     Baseline loss EMA: {self.loss_ema:.4f}")
...
class OnlineAdvantageFilter:
    ...
    def __init__(
        ...
        advantage_threshold: float = 0.1,  # Start lower, increase with progress
        advantage_threshold_max: float = 0.3,  # Maximum threshold at end of training
        skip_easy_batches: bool = True,
        max_consecutive_skips: int = 10,
        dynamic_threshold: bool = True,  # Enable dynamic threshold scaling
    ):
        self.loss_threshold = loss_threshold
        self.advantage_threshold = advantage_threshold
        self.advantage_threshold_min = advantage_threshold
        self.advantage_threshold_max = advantage_threshold_max
        ...
        self._training_progress = 0.0  # 0.0 to 1.0

        # TSF Integration
        self.tsf = None
        self.recent_advantages = []
        if TSF_AVAILABLE:
            self.tsf = TrajectorySegmentFilter(
                segment_length=64,
                threshold_min=2.0,  # Sum of 64 batches * 0.03 avg advantage
                threshold_max=20.0,  # Sum of 64 batches * 0.3 avg advantage
            )
            print(
                f"✅ TrajectorySegmentFilter integrated (len=64, min={self.tsf.threshold_min}, max={self.tsf.threshold_max})"
            )

        print(f"✅ OnlineAdvantageFilter initialized")
        print(f"   Loss threshold: {loss_threshold}")
        print(
            f"   Advantage threshold: {advantage_threshold} (dynamic: {dynamic_threshold})"
        )
        if dynamic_threshold:
            print(
                f"   Threshold range: {advantage_threshold} → {advantage_threshold_max}"
            )

    def update_training_progress(self, progress: float):
        """
        Update training progress for dynamic threshold scaling.

        Args:
            progress: Training progress from 0.0 to 1.0 (epoch/total_epochs)
        """
        self._training_progress = max(0.0, min(1.0, progress))
        if self.dynamic_threshold:
            # Linear interpolation from min to max threshold
            self.advantage_threshold = (
                self.advantage_threshold_min
                + (self.advantage_threshold_max - self.advantage_threshold_min)
    ...
    def analyze_batch(
        ...
        # TSF Logic: Update dynamic threshold based on trajectory
        if self.tsf:
            self.recent_advantages.append(advantage_score)
            if len(self.recent_advantages) > self.tsf.segment_length:
                self.recent_advantages.pop(0)

            if len(self.recent_advantages) == self.tsf.segment_length:
                segment_sum = sum(self.recent_advantages)
                # If segment is too easy (low sum), increase threshold to skip more
                if segment_sum < self.tsf.threshold_min:
                    self.advantage_threshold = min(
                        self.advantage_threshold_max, self.advantage_threshold * 1.05
                    )
                # If segment is hard (high sum), decrease threshold to learn more
                elif segment_sum > self.tsf.threshold_max:
                    self.advantage_threshold = max(
                        self.advantage_threshold_min, self.advantage_threshold * 0.95
                    )
            ...
            stages=[
                ...
                SkillStage(
                    ...
                    loss_weights={
                        ...
                        "box": 8.0,  # Fixed: was 12.0, now closer to YOLOv8 default (7.5) for stability
                        "dfl": 2.5,  # Fixed: was 3.0, slightly lower for stability
                    },
...
def _train_multiscale(
    ...
    for i, phase in enumerate(config.multiscale_phases):
        ...
        train_args = {
            ...
            "amp": config.amp,
            "cache": config.cache,
            # NOTE: EMA (Exponential Moving Average) is enabled by default in ultralytics 8.x
            # and cannot be disabled. The 'ema' argument has been removed.
            # EMA provides +2-3% mAP50 improvement for stable convergence.
            # Validation
            "val": True,
...
def main():
    ...
    # Signal training complete - allow coding model to reclaim GPU
    _signal_training_complete()


def _signal_training_complete():
    """Signal to coding model that training is complete and GPU can be reclaimed."""
    import urllib.request
    import urllib.error
    ...
    urls_to_try = [
        "http://coding-model-primary:8000",
        "http://shml-coding-model-primary:8000",
        "http://localhost:8000",
    ]

    for base_url in urls_to_try:
        try:
            url = f"{base_url}/training/stop?job_id={job_id}"
            req = urllib.request.Request(url, method="POST")

            print(f"[GPU-RECLAIM] Signaling training complete to {base_url}...")
            with urllib.request.urlopen(req, timeout=10) as response:
                result = response.read().decode("utf-8")
                print(f"[GPU-RECLAIM] ✓ GPU reclaim signaled: {result}")
                return True
    ...
    print(
        "[GPU-RECLAIM] Could not reach coding model service - GPU will reclaim on timeout"
    )
    return False
