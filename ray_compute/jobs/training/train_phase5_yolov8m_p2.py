#!/usr/bin/env python3
"""
Phase 5 Face Detection - YOLOv8m-P2 (OPTIMIZED TRANSFER LEARNING)
==================================================================

CRITICAL FIX: Phase 4 trained FROM SCRATCH instead of from Phase 3 checkpoint!
This explains the 4.14% mAP regression and 9.52% recall drop.

Root Cause Analysis (Phase 4 vs Phase 3):
┌─────────────────────┬──────────────────────┬────────────────────────┐
│ Parameter           │ Phase 4 (WRONG)      │ Phase 5 (FIXED)        │
├─────────────────────┼──────────────────────┼────────────────────────┤
│ Base Model          │ yolov8m-face-p2.yaml │ Phase 3 best.pt        │
│ Transfer Learning   │ ❌ FROM SCRATCH      │ ✅ From Phase 3        │
│ Optimizer           │ SGD                  │ AdamW                  │
│ Learning Rate       │ 0.005                │ 0.001 (fine-tuning)    │
│ Mosaic Augmentation │ ❌ DISABLED (0.0)    │ ✅ ENABLED (1.0)       │
│ MixUp               │ ❌ DISABLED (0.0)    │ ✅ ENABLED (0.15)      │
│ close_mosaic        │ Not set              │ 15 epochs before end   │
│ patience            │ 20                   │ 50 (don't stop early)  │
└─────────────────────┴──────────────────────┴────────────────────────┘
...
1. Transfer Learning: Start from Phase 3 checkpoint (43.6M params → 19M params adapter)
2. AdamW Optimizer: Better for fine-tuning, used successfully in Phase 3
3. Cosine LR Schedule: 0.001 → 0.00001 (10x lower for transfer learning)
4. Mosaic Augmentation: Re-enabled (was accidentally disabled in Phase 4)
5. MixUp: 15% probability for regularization
6. Gradient Accumulation: nbs=64 means effective batch = 64 (accumulates 16 steps)
7. Longer Training: 200 epochs with patience=50 (don't early stop too soon)
8. Label Smoothing: 0.1 for better generalization (from ML Engineering book)

Expected Improvements:
- mAP50: 80.64% → 86%+ (recover Phase 3's 84.78% + P2 advantage)
- Recall: 64.74% → 78%+ (recover Phase 3's 74.26% + improvement)
- Precision: Maintain ~93% (Phase 4 was already good here)
...
_job_id = os.environ.get("RAY_JOB_ID", f"phase5-yolov8m-{os.getpid()}")
...
MLFLOW_CONFIG = {
    ...
    "experiment_name": "Face-Detection-P2",
...
# CRITICAL: Phase 3 checkpoint as base (NOT training from scratch!)
PHASE3_WEIGHTS = CHECKPOINT_DIR / "phase_3_phase_3/weights/best.pt"

# Output directory
TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
OUTPUT_NAME = f"phase_5_yolov8m_p2_{TIMESTAMP}"
...
def check_prerequisites():
    ...
    checks = [
        ...
        ("Phase 3 weights (BASE)", PHASE3_WEIGHTS),  # CRITICAL!
...
def run_training(dry_run: bool = False, epochs: int = None):
    ...
    print("=" * 70)
    print("STARTING PHASE 5 TRAINING (Transfer Learning)")
    print("=" * 70)
    ...
    # CRITICAL: Load Phase 3 model as base
    print(f"\n📦 Loading Phase 3 checkpoint: {PHASE3_WEIGHTS}")
    print("   This is the key difference from Phase 4!")

    # Load the Phase 3 model
    # The model will be modified to use P2 architecture while keeping learned weights
    ...
    print(f"   Using transfer learning from Phase 3 (NOT from scratch!)")
    print(f"   Mosaic: {train_config['mosaic']} (was 0.0 in Phase 4)")
