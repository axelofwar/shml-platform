...
# ═══════════════════════════════════════════════════════════════════════════
#
# Runs Qwopus and Qwen3.5 sequentially on GPU 0, collecting full eval
...
QWOPUS_COMPOSE="$PLATFORM_ROOT/inference/qwopus/docker-compose.yml"
...
QWOPUS_EVAL_PORT=8010
QWEN_EVAL_PORT=8020
QWOPUS_INTERNAL_PORT=8000
...
start_model_for_eval() {
    ...
    if [[ "$name" == "qwopus" ]]; then
        ...
        docker run -d --name "qwopus-eval-proxy" \
            ...
            "TCP:${container}:${internal_port}" 2>/dev/null || {
                ...
                QWOPUS_EVAL_PORT_OVERRIDE="${port}:${internal_port}" \
...
cleanup_qwopus_proxy() {
    docker rm -f qwopus-eval-proxy 2>/dev/null || true
}
...
main() {
    ...
    stop_model "$QWOPUS_COMPOSE" "Qwopus"
    ...
    if [[ "$SKIP_QWOPUS" == true ]]; then
        ...
        log "Phase 2: Evaluating Qwopus (Qwen3.5-27B Q4_K_M)"

        start_model_for_eval "$QWOPUS_COMPOSE" "qwopus" "$QWOPUS_EVAL_PORT" "$QWOPUS_INTERNAL_PORT"

        python3 "$EVAL_SCRIPT" \
            --target qwopus \
            --qwopus-url "http://localhost:${QWOPUS_EVAL_PORT}/v1" \
            --category "$CATEGORY" \
...
" 2>&1 | tee "$RESULTS_DIR/comparison_${TIMESTAMP}.txt"
    ...
    if [[ "$NO_RESTART" == true ]]; then
        ...
        if [[ "$restart_target" == "qwopus" ]]; then
            docker compose -f "$QWOPUS_COMPOSE" up -d 2>&1
            wait_for_health "http://localhost:${QWOPUS_EVAL_PORT}/health" "Qwopus" 120 || \
