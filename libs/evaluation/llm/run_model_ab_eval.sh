...
# ═══════════════════════════════════════════════════════════════════════════
#
# Runs Qwopus (Qwen3.5-27B) and Qwen3 sequentially on GPU 0, collecting full eval
...
#   2. Start Qwopus → wait for health → run eval → save results → stop
#   3. Start Qwen → wait for health → run eval → save results → stop
#   4. Merge results and print comparison
#   5. Restart the winner (or Qwopus by default)
#
# Usage:
...
#   --restart MODEL        Which model to restart: winner|qwopus|qwen (default: qwopus)
#   --skip-qwopus          Skip Qwopus eval (use existing results)
#   --skip-qwen            Skip Qwen eval (use existing results)
#   --timeout SECS         Health check timeout in seconds (default: 180)
#
# Requirements:
#   - Docker with NVIDIA runtime
#   - Qwopus GGUF at data/models/qwopus/
...
CODING_COMPOSE="$PLATFORM_ROOT/inference/qwopus/docker-compose.yml"
QWEN_COMPOSE="$PLATFORM_ROOT/inference/qwen/docker-compose.yml"

# Qwopus uses Traefik (no direct port), so we expose it temporarily
CODING_EVAL_PORT=8010
QWEN_EVAL_PORT=8020
CODING_INTERNAL_PORT=8000
QWEN_INTERNAL_PORT=8000

CATEGORY="all"
RESTART_MODEL="qwopus"
SKIP_QWOPUS=false
SKIP_QWEN=false
HEALTH_TIMEOUT=180
NO_RESTART=false
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --skip-qwopus) SKIP_QWOPUS=true; shift ;;
...
start_model_for_eval() {
    ...
    # For Qwopus: it doesn't expose ports directly (Traefik-only),
    ...
    local health_url
    if [[ "$name" == "qwopus" ]]; then
        # Qwopus doesn't expose ports directly; access via container network
        ...
        docker run -d --name "coding-eval-proxy" \
            ...
            "TCP:${container}:${internal_port}" 2>/dev/null || {
                ...
                CODING_EVAL_PORT_OVERRIDE="${port}:${internal_port}" \
                    docker compose -f "$compose_file" up -d \
                    --scale coding-manager=0 2>&1
            }
...
cleanup_coding_proxy() {
    docker rm -f coding-eval-proxy 2>/dev/null || true
}
...
main() {
    ...
    stop_model "$CODING_COMPOSE" "Qwopus"
    ...
    # ── Phase 2: Evaluate Qwopus ──

    QWOPUS_RESULTS="$RESULTS_DIR/qwopus_${TIMESTAMP}.json"

    if [[ "$SKIP_QWOPUS" == true ]]; then
        warn "Skipping Qwopus evaluation (--skip-qwopus)"
        # Find latest qwopus results
        QWOPUS_RESULTS=$(ls -t "$RESULTS_DIR"/qwopus_*.json 2>/dev/null | head -1 || echo "")
        if [[ -z "$QWOPUS_RESULTS" ]]; then
            warn "No existing Qwopus results found"
        fi
    else
        log "Phase 2: Evaluating Qwopus (Qwen3.5-27B)"

        start_model_for_eval "$CODING_COMPOSE" "qwopus" "$CODING_EVAL_PORT" "$CODING_INTERNAL_PORT"

        python3 "$EVAL_SCRIPT" \
            --target qwopus \
            --coding-url "http://localhost:${CODING_EVAL_PORT}/v1" \
            --category "$CATEGORY" \
            --output "$QWOPUS_RESULTS" \
            2>&1 | tee "$RESULTS_DIR/qwopus_${TIMESTAMP}.log"

        ok "Qwopus results saved: $QWOPUS_RESULTS"

        # Stop Qwopus and free GPU
        stop_model "$CODING_COMPOSE" "Qwopus"
        cleanup_coding_proxy
    ...
    if [[ -n "$QWOPUS_RESULTS" && -f "$QWOPUS_RESULTS" && \
...
with open('$QWOPUS_RESULTS') as f:
    qwopus = json.load(f)
with open('$QWEN_RESULTS') as f:
    qwen = json.load(f)

n_results = qwopus.get('results', {}).get('qwopus', [])
...
for r in n_results:
    ...
    categories.setdefault(cat, {'qwopus': [], 'qwen': []})
    categories[cat]['qwopus'].append(r)
for r in q_results:
    cat = r.get('category', 'unknown')
    categories.setdefault(cat, {'qwopus': [], 'qwen': []})
...
for cat, data in sorted(categories.items()):
    print(f'\n  ── {cat.upper()} ──')
    print(f\"  {'Task':<35s} {'Qwopus':>10s} {'Qwen':>10s} {'Winner':>10s}\")
    print(f'  {\"─\" * 67}')

    for nr in data['qwopus']:
        task_id = nr['task_id']
...
print()
print('=' * 80)
print(f'  OVERALL: Qwopus {n_total_passed}/{n_total_checks} ({n_pct:.0f}%) | Qwen {q_total_passed}/{q_total_checks} ({q_pct:.0f}%)')
winner = 'Qwen' if q_pct > n_pct else 'Qwopus' if n_pct > q_pct else 'Tie'
...
" 2>&1 | tee "$RESULTS_DIR/comparison_${TIMESTAMP}.txt"
    ...
    if [[ "$NO_RESTART" == true ]]; then
        ...
        if [[ "$restart_target" == "qwopus" ]]; then
            docker compose -f "$CODING_COMPOSE" up -d 2>&1
            wait_for_health "http://localhost:${CODING_EVAL_PORT}/health" "Qwopus" 120 || \
                warn "Qwopus restart may not have direct port access (Traefik-only)"
    ...
    echo "  Results:    $RESULTS_DIR/"
    echo "  Qwopus:     ${QWOPUS_RESULTS:-N/A}"
