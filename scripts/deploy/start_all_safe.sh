...
start_all_services() {
    ...
    # =========================================================================
    if [ -f "$PROJECT_ROOT/scripts/platform/env-init.sh" ]; then
        bash "$PROJECT_ROOT/scripts/platform/env-init.sh"
    ...
    if ! docker compose --env-file .env -f deploy/compose/docker-compose.auth.yml up -d --force-recreate --build role-auth 2>&1 | grep -v "orphan"; then
        log_error "FATAL: role-auth failed to start — all developer-facing services (GitLab, Grafana, etc.) will return 502"
        log_warn "Fix: docker compose --env-file .env -f deploy/compose/docker-compose.auth.yml up -d --build role-auth"
        # Continue startup — other services can still run, but dev access will be broken
    fi

    if ! wait_for_health "role-auth" $DEFAULT_TIMEOUT; then
        log_error "CRITICAL: role-auth container failed health check — RBAC middlewares will be unavailable"
        log_warn "Services protected by role-auth-developer/admin will return 502 after login"
        log_warn "Fix: docker compose --env-file .env -f deploy/compose/docker-compose.auth.yml up -d --build role-auth"
    fi

    # Wait for role-auth middlewares to be registered with Traefik
    if ! wait_for_middleware "role-auth-developer" 30; then
        log_error "role-auth-developer middleware not registered in Traefik — GitLab, Grafana, and all developer services inaccessible"
    fi
    if ! wait_for_middleware "role-auth-admin" 30; then
        log_warn "role-auth-admin middleware not registered in Traefik"
    fi
    ...
    if [ "$OPTIONAL_AI_STACK_ENABLED" != "true" ]; then
        ...
        if [ -f "inference/qwopus/docker-compose.yml" ]; then
            ...
            echo "  Waiting for Qwopus coding model (Qwen3.5-27B on RTX 3090Ti)..."
            wait_for_health "qwopus-coding" $CODING_MODEL_TIMEOUT || log_warn "Qwopus may still be loading (this can take 2-5 minutes)"
        else
            log_warn "Qwopus configuration not found - skipping primary coding model"
        fi
    ...
    LLAMA_SCRIPT="${PROJECT_ROOT}/inference/llama-cpp/start-qwen35-cuda.sh"
    LLAMA_PID_FILE="${PROJECT_ROOT}/inference/llama-cpp/qwen35-server.pid"
    if [ "$OPTIONAL_AI_STACK_ENABLED" != "true" ]; then
    ...
    if [ -x "${PROJECT_ROOT}/scripts/generate_container_mapping.sh" ]; then
        "${PROJECT_ROOT}/scripts/generate_container_mapping.sh" >/dev/null 2>&1 && \
...
update_container_mapping() {
    ...
    if [ -x "${PROJECT_ROOT}/scripts/generate_container_mapping.sh" ]; then
        "${PROJECT_ROOT}/scripts/generate_container_mapping.sh" >/dev/null 2>&1 || true
    fi
    # Generate Prometheus metrics file for container name labels
    if [ -x "${PROJECT_ROOT}/scripts/generate_container_name_metrics.sh" ]; then
        "${PROJECT_ROOT}/scripts/generate_container_name_metrics.sh" >/dev/null 2>&1 || true
    fi
}
...
start_auth() {
    ...
    if ! docker ps --format '{{.Names}}' | grep -q "^${PLATFORM_PREFIX:-shml}-postgres$" || \
       ! docker ps --format '{{.Names}}' | grep -q "^${PLATFORM_PREFIX:-shml}-redis$" || \
       ! docker ps --format '{{.Names}}' | grep -q "^${PLATFORM_PREFIX:-shml}-traefik$"; then
        log_warn "Core infrastructure not running - starting infrastructure first"
        start_infra || {
            log_error "Infrastructure failed to start"
            return 1
        }
    fi
    ...
    wait_for_middleware "oauth2-auth" 60 || {
        log_error "oauth2-auth middleware failed to register"
        return 1
    }
    update_container_mapping
...
start_inference() {
    ...
    # Qwopus coding model (PRIMARY - RTX 3090 Ti)
    # Qwen3.5-27B Q4_K_M — primary coding model
    if [ -f "inference/qwopus/docker-compose.yml" ]; then
        echo "Starting Qwopus coding model (Qwen3.5-27B on RTX 3090 Ti)..."
        dc_up inference/qwopus/docker-compose.yml --force-recreate
        wait_for_health "qwopus-coding" ${CODING_MODEL_TIMEOUT:-300} || log_warn "Qwopus may still be loading (~16GB model)"
    fi
    ...
    if [ -f "inference/coding-model/docker-compose.yml" ]; then
        ...
        dc_up inference/coding-model/docker-compose.yml --force-recreate
        wait_for_health "coding-model-fallback" 180 || log_warn "Fallback model may still be loading"
        # Note: coding-model-primary is now deprecated, replaced by Qwopus
    fi
...
hydrate_gitlab_runner_tokens() {
    local bootstrap_script="${PROJECT_ROOT}/scripts/platform/gitlab_runner_bootstrap.py"
    if [ ! -f "$bootstrap_script" ]; then
        log_warn "GitLab runner bootstrap helper not found - relying on existing runner configs"
        return 1
    fi

    if ! command -v python3 >/dev/null 2>&1; then
        log_warn "python3 not available - cannot auto-fetch GitLab runner registration tokens"
        return 1
    fi

    local export_blob
    if ! export_blob=$(python3 "$bootstrap_script" export-env 2>/dev/null); then
        log_warn "Unable to auto-fetch GitLab runner tokens - relying on existing runner configs"
        return 1
    fi

    eval "$export_blob"
    log_success "Hydrated per-project GitLab runner registration tokens"
    return 0
}

start_devtools() {
    log_info "━━━ Starting Development Tools (DevTools Layer) ━━━"
    local devtools_failed=0
    hydrate_gitlab_runner_tokens || true

    # Verify prerequisites (OAuth middleware must exist)
    if ! curl -sf "http://localhost:8090/api/http/middlewares/oauth2-auth@docker" >/dev/null 2>&1; then
        log_warn "oauth2-auth middleware not found - starting auth services first"
        start_auth || {
            log_error "Auth services failed to start"
            return 1
        }
        wait_for_middleware "oauth2-auth" 60 || {
            log_error "oauth2-auth middleware failed to register"
    ...
    if ! curl -sf "http://localhost:8090/api/http/middlewares/role-auth-admin@docker" >/dev/null 2>&1; then
        ...
        dc_up deploy/compose/docker-compose.auth.yml role-auth || {
            log_error "role-auth failed to start"
            return 1
        }
        wait_for_middleware "role-auth-admin" 30 || {
            log_error "role-auth-admin middleware failed to register"
            return 1
        }
    fi

    dc_up deploy/compose/docker-compose.devtools.yml || {
        log_error "Development tools compose startup failed"
        return 1
    }

    wait_for_health "${PLATFORM_PREFIX:-shml}-code-server" "${CODE_SERVER_TIMEOUT:-120}" 1 || {
        log_error "Code Server failed to become healthy"
        devtools_failed=1
    }
    wait_for_health "${PLATFORM_PREFIX:-shml}-gitlab" "${GITLAB_TIMEOUT:-420}" 1 || {
        log_error "GitLab failed to become healthy"
        devtools_failed=1
    }
    wait_for_health "${PLATFORM_PREFIX:-shml}-sba-resource-portal" "${SBA_PORTAL_TIMEOUT:-60}" 1 || {
        log_error "SBA Portal failed to become healthy"
        devtools_failed=1
    }

    local runner_names=(platform robotics training)
    local runner_name
    for runner_name in "${runner_names[@]}"; do
        ...
        runner_suffix=$(printf '%s' "$runner_name" | tr '[:lower:]' '[:upper:]')
        token_var="GITLAB_${runner_suffix}_RUNNER_REGISTRATION_TOKEN"
        runner_container="${PLATFORM_PREFIX:-shml}-gitlab-runner-${runner_name}"
        token_value="${!token_var:-}"

        if docker inspect "$runner_container" >/dev/null 2>&1; then
            # Container exists — ensure it is actually running (not stuck in "Created" state)
            local runner_actual_status
            runner_actual_status=$(docker inspect --format='{{.State.Status}}' "$runner_container" 2>/dev/null) || runner_actual_status=""
            if [[ "$runner_actual_status" == "created" ]]; then
                log_info "Runner ${runner_container} is in 'created' state — starting it now"
                docker start "$runner_container" >/dev/null 2>&1 || log_warn "Failed to start ${runner_container}"
            fi

            if [ -n "$token_value" ]; then
                wait_for_health "$runner_container" "${GITLAB_RUNNER_TIMEOUT:-60}" 1 || {
                    log_error "${runner_container} failed to start with an auto-fetched registration token"
                    devtools_failed=1
                }
            elif ! docker ps --format '{{.Names}}' | grep -q "^${runner_container}$"; then
                log_warn "${runner_container} is not running; it needs either an existing config volume or a readable GitLab API token"
            fi
        elif [ -n "$token_value" ]; then
            log_error "${runner_container} was not created despite an available registration token"
            devtools_failed=1
        else
            log_warn "${runner_container} was skipped; configure GITLAB_API_TOKEN or seed its config volume manually"
        fi
    done

    if [ "$devtools_failed" -ne 0 ]; then
        log_error "Development tools stack is not healthy after startup"
        return 1
    fi
...
start_watchdog() {
    ...
    dc_up deploy/compose/docker-compose.watchdog.yml || {
        log_error "Watchdog compose startup failed"
        return 1
    }
    sleep 3
    if docker ps --format '{{.Names}}' | grep -q "^${PLATFORM_PREFIX:-shml}-watchdog$"; then
        log_success "Watchdog started"
        return 0
    fi

    log_error "Watchdog container did not start"
    return 1
}
...
start_sba_portal() {
    ...
    if ! curl -sf "http://localhost:8090/api/http/middlewares/role-auth-developer@docker" >/dev/null 2>&1; then
        ...
        wait_for_middleware "role-auth-developer" 30 || {
            log_error "role-auth-developer middleware failed to register"
            return 1
        }
    fi
    ...
    dc_up deploy/compose/docker-compose.devtools.yml sba-resource-portal || {
        log_error "SBA Resource Portal compose startup failed"
        return 1
    }
    wait_for_health "${PLATFORM_PREFIX:-shml}-sba-resource-portal" "${SBA_PORTAL_TIMEOUT:-60}" 1 || {
        log_error "SBA Portal failed to become healthy"
