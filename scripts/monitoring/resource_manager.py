...
class ResourceManager:
    ...
    def calculate_allocations(
        self, system_resources: Dict[str, float], current_config: Dict[str, Any]
        ...
        for service_name in services.keys():
            min_req = self.MIN_REQUIREMENTS.get(
                service_name, {"cpus": 0.1, "memory_mb": 128}
            )
        ...
        for service_name in services.keys():
            ...
            memory_mb = min_req["memory_mb"] + (
                remaining_memory_gb * 1024 * weight_ratio
            )
    ...
    def apply_allocations(
        self, config: Dict[str, Any], allocations: Dict[str, Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for service_name, allocation in allocations.items():
            if service_name in config["services"]:
                ...
                service["deploy"]["resources"]["limits"]["cpus"] = str(
                    allocation["cpus_limit"]
                )
                service["deploy"]["resources"]["limits"]["memory"] = allocation[
                    "memory_limit"
                ]
                service["deploy"]["resources"]["reservations"]["cpus"] = str(
                    allocation["cpus_reservation"]
                )
                service["deploy"]["resources"]["reservations"]["memory"] = allocation[
                    "memory_reservation"
                ]
    ...
    def generate_report(
        self, system_resources: Dict[str, float], allocations: Dict[str, Dict[str, Any]]
    ) -> str:
        ...
        lines.append(
            f"  Available CPUs:       {system_resources['available_cpus']:.2f} (80% of total)"
        )
        lines.append(
            f"  Total Memory:         {system_resources['total_memory_gb']:.2f}GB"
        )
        lines.append(
            f"  Available Memory:     {system_resources['available_memory_gb']:.2f}GB (80% of available)"
        )
        lines.append(
            f"  Memory Usage:         {system_resources['memory_used_percent']:.1f}%"
        )
        lines.append(
            f"  Swap Usage:           {system_resources['swap_used_percent']:.1f}%"
        )
        ...
        lines.append("")
        lines.append(
            f"{'Service':<25} {'CPU Limit':<12} {'CPU Reserve':<12} {'Mem Limit':<12} {'Mem Reserve':<12}"
        )
        lines.append("-" * 80)
        ...
        lines.append("-" * 80)
        lines.append(
            f"{'TOTAL':<25} {total_cpu_limits:<12.2f} {total_cpu_reserves:<12.2f}"
        )
        lines.append("")
    ...
    def run(self, dry_run: bool = False) -> Tuple[bool, str]:
        ...
        try:
            ...
            print(
                f"  cd {self.compose_file.parent} && docker-compose down && docker-compose up -d"
            )
