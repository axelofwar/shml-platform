...
class TestSchemaEndpoints:
    ...
    def test_get_experiment_schema(self, api_v1_url, test_experiment_name):
        ...
        response = requests.get(
            f"{api_v1_url}/schema/experiment/{test_experiment_name}", timeout=10
        )
    ...
    def test_validate_schema(self, api_v1_url, test_tags):
        ...
        response = requests.post(
            f"{api_v1_url}/schema/validate", json=payload, timeout=10
        )
...
class TestRunEndpoints:
    ...
    @pytest.mark.integration
    def test_create_run_with_complete_tags(
        self, api_v1_url, test_experiment_name, test_tags
    ):
    ...
    def test_create_run_with_incomplete_tags_shows_warnings(
        ...
        assert any(
            word in warnings_text
            for word in ["recommend", "missing", "info", "schema", "tag"]
        )
    ...
    def test_log_metrics(self, api_v1_url, created_run):
        ...
        payload = {"metrics": {"accuracy": 0.95, "loss": 0.05}, "step": 1}

        response = requests.post(
            f"{api_v1_url}/runs/{run_id}/metrics", json=payload, timeout=10
        )
...
class TestArtifactEndpoints:
    ...
    def test_download_artifact(self, api_v1_url, run_with_artifacts):
        ...
        response = requests.get(
            f"{api_v1_url}/runs/{run_id}/artifacts/test_artifacts/test.txt", timeout=30
        )
...
class TestModelRegistryEndpoints:
    ...
    def test_register_model_with_low_metrics_shows_warnings(
        ...
        response = requests.post(
            f"{api_v1_url}/models/register", json=payload, timeout=30
        )
        ...
        warnings_text = " ".join(data["warnings"]).lower()
        assert (
            "privacy" in warnings_text
            or "recall" in warnings_text
            or "recommend" in warnings_text
        )
    ...
    def test_register_model_complete(self, api_v1_url, run_with_model):
        ...
        payload = {
            ...
            "tags": {"environment": "test", "model_type": "classification"},
