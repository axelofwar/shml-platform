...
def wait_for_job(
    client: JobSubmissionClient, job_id: str, timeout: int = 300, poll_interval: int = 5
) -> Tuple[str, str]:
...
class RayGPUTester:
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            print(
                f"  Active jobs: {len([j for j in jobs if j.status == JobStatus.RUNNING])}"
            )
            return True
        except Exception as e:
...
"""
        ...
        success, logs = self.submit_and_wait(
            "resource_check", code, num_gpus=0, timeout=60
        )
...
"""
        ...
        success, logs = self.submit_and_wait(
            "gpu_detection", code, num_gpus=1, timeout=120
        )
...
"""
        ...
        success, logs = self.submit_and_wait(
            "single_gpu", code, num_gpus=1, timeout=120
        )
...
"""
        ...
        success, logs = self.submit_and_wait(
            "fractional_gpu", code, num_gpus=0.5, timeout=120
        )
...
"""
    ...
    def run_tests(
        self, quick: bool = False, full: bool = False, gpu_mode: str = "all"
    ) -> Dict[str, bool]:
        """Run test suite"""
...
""",
    ...
    parser.add_argument(
        "--full", action="store_true", help="Full test including training"
    )
    parser.add_argument(
        "--gpu-mode",
    ...
    # Run tests
    results = tester.run_tests(quick=args.quick, full=args.full, gpu_mode=args.gpu_mode)
