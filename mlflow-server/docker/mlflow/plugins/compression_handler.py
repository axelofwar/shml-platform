...
class CompressionConfig:
    ...
    COMPRESSIBLE_EXTENSIONS = {
        '.pt', '.pth', '.h5', '.pkl', '.onnx', '.pb', '.safetensors',  # Models
        '.csv', '.json', '.yaml', '.yml', '.txt', '.log',  # Data/Config
        '.npy', '.npz', '.parquet',  # Numpy/Pandas
        '.jpg', '.jpeg', '.png',  # Images (re-compress with zstd)
    }
    
    # File types to never compress (already compressed)
    SKIP_COMPRESSION = {
        '.zst', '.gz', '.zip', '.tar.gz', '.tgz', '.bz2', '.xz', '.7z'
    }
...
class ArtifactCompressor:
    ...
    def should_compress(self, file_path: Path) -> bool:
        ...
        size_mb = file_path.stat().st_size / (1024 * 1024)
        if size_mb < self.config.THRESHOLD_MB:
            logger.debug(f"Skipping {file_path.name} - below threshold ({size_mb:.2f}MB)")
            return False
    ...
    def compress_file(self, input_path: Path, output_path: Optional[Path] = None) -> Tuple[Path, dict]:
        """
        ...
        logger.info(f"Compressing {input_path.name} ({original_size / 1024 / 1024:.2f}MB)...")
    ...
    def decompress_file(self, input_path: Path, output_path: Optional[Path] = None) -> Path:
        ...
        logger.info(f"Decompressing {input_path.name} ({compressed_size / 1024 / 1024:.2f}MB)...")
