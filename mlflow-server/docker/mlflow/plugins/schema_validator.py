...
class RequiredMetadata(BaseModel):
    ...
    model_name: str = Field(..., description="Model name in format: {project}_{architecture}_{purpose}")
    model_version: str = Field(..., pattern=r"^v\d+\.\d+\.\d+$", description="Semantic version (v1.0.0)")
    author: str = Field(..., description="Model creator username")
    description: str = Field(..., min_length=10, description="Model description (min 10 chars)")
    ...
    model_type: str = Field(..., description="detection|classification|regression|segmentation")
    
    # Dataset info
    dataset_name: str = Field(..., description="Name of training dataset")
    dataset_version: str = Field(..., description="Dataset version (v2024.11.21 or vX.Y.Z)")
    dataset_path: Optional[str] = Field(None, description="Path to dataset (if using reference)")
    ...
    input_shape: str = Field(..., description="Expected input shape (e.g., '[3,224,224]')")
    ...
    def validate_task_metrics(self, model_type: str):
        ...
        if model_type in ['classification', 'detection']:
            if not any([self.accuracy, self.precision, self.recall, self.f1_score]):
                raise ValueError(f"{model_type} requires at least one of: accuracy, precision, recall, f1_score")
            ...
            if not any([self.rmse, self.mae, self.r2_score]):
                raise ValueError("Regression models require at least one of: rmse, mae, r2_score")
...
class RequiredArtifacts(BaseModel):
    ...
    requirements_file: str = Field(..., description="Python dependencies (requirements.txt)")
    ...
    preprocessing_script: Optional[str] = Field(None, description="Preprocessing code (.py)")
    postprocessing_script: Optional[str] = Field(None, description="Postprocessing code (.py)")
    training_script: Optional[str] = Field(None, description="Training code (.py)")
    
    # Privacy-aware: Allow reference paths
    use_reference_paths: bool = Field(False, description="If True, paths point to external storage")
...
class SchemaValidator:
    ...
    def validate_run(self, run_id: str, allow_warnings: bool = True) -> Dict[str, Any]:
        ...
        required_artifacts = ['model', 'config', 'requirements.txt', 'metrics.json', 
                            'sample_input', 'sample_output']
        missing_artifacts = [a for a in required_artifacts if not any(a in p for p in artifact_paths)]
    ...
    def check_model_completeness(self, run_id: str, check_download: bool = True) -> Dict[str, Any]:
        """
        ...
        for component, patterns in essential.items():
            if not any(any(p in artifact for artifact in artifact_paths) for p in patterns):
        ...
        if not any('sample_input' in a for a in artifact_paths):
            ...
            results["warnings"].append("Missing sample_input - model validation will be difficult")
        
        if not any('sample_output' in a for a in artifact_paths):
            results["warnings"] = results.get("warnings", [])
            results["warnings"].append("Missing sample_output - cannot verify model output format")
