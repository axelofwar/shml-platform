...
def verify_features(client):
    ...
    features['schema_validation'] = {
        ...
        'message': '✓ Schema validation enabled' if schema_enabled else '✗ Schema validation disabled'
    }
    ...
    features['compression'] = {
        ...
        'message': f'✓ Auto-compression enabled ({os.getenv("MLFLOW_COMPRESSION_FORMAT", "zstd")})' if compression else '✗ Compression disabled'
    }
...
def main():
    ...
    for feature_name, feature_data in features.items():
        if feature_data.get('available'):
            print(f"✅ {feature_name.replace('_', ' ').title()}: {feature_data.get('message', 'Available')}")
        else:
            print(f"❌ {feature_name.replace('_', ' ').title()}: {feature_data.get('error', feature_data.get('message', 'Not available'))}")
...
""")
    ...
    print(f"  List models:          GET  {base_url}/api/2.0/mlflow/registered-models/list")
    print(f"  Search model versions: POST {base_url}/api/2.0/mlflow/model-versions/search")
