...
from fastapi import FastAPI, HTTPException, UploadFile, File, Form, Path as PathParam, Query
...
class RunCreateRequest(BaseModel):
    ...
    experiment_name: str = Field(..., description="Name of experiment (e.g., 'Development-Training')")
    ...
    parameters: Optional[Dict[str, Any]] = Field(default={}, description="Initial parameters to log")
...
class ModelRegisterRequest(BaseModel):
    ...
    privacy_validated: bool = Field(default=False, description="Whether model passed privacy validation (recommended)")
    min_recall: Optional[float] = Field(None, description="Target recall threshold for alerts (e.g., 0.95)")
...
def validate_run_against_schema(experiment_name: str, tags: Dict[str, str], schema: Dict[str, Any]) -> tuple[bool, List[str]]:
    """
    ...
    if not exp_schema:
        ...
        warnings.append(f"INFO: Experiment '{experiment_name}' not in schema - this is OK")
    ...
    for req_tag in required_tags:
        if req_tag not in tags:
            warnings.append(f"RECOMMENDED: Add tag '{req_tag}' for better experiment tracking")
...
@app.get("/api/v1/schema/experiment/{experiment_name}", summary="Get schema for specific experiment")
async def get_experiment_schema(
    experiment_name: str = PathParam(..., description="Experiment name (e.g., 'Development-Training')")
):
    """
...
async def validate_against_schema(request: dict):
    ...
    try:
        ...
        schema = load_schema()
        is_valid, warnings = validate_run_against_schema(experiment_name, tags_dict, schema)
...
async def upload_artifact(
    ...
    artifact_path: Optional[str] = Form(default="", description="Subpath within artifacts (e.g., 'plots')")
):
    """
...
@app.get("/api/v1/runs/{run_id}/artifacts/{artifact_path:path}", summary="Download artifact")
async def download_artifact(
    ...
    try:
        ...
        if os.path.isdir(local_path):
            ...
            for item in os.listdir(local_path):
                ...
                contents.append({
                    ...
                    "file_size": os.path.getsize(item_path) if os.path.isfile(item_path) else None
                })
            return {
                "path": artifact_path,
                "is_directory": True,
        ...
        return format_error_response(e, f"/api/v1/runs/{run_id}/artifacts/{artifact_path}")
...
async def register_model(request: ModelRegisterRequest):
    ...
    try:
        ...
        if not request.privacy_validated:
            warnings.append("RECOMMENDED: Privacy validation not completed. Consider validating before production deployment.")
...
@app.post("/api/v1/models/{model_name}/versions/{version}/transition", summary="Transition model stage")
async def transition_model_stage(
    model_name: str = PathParam(..., description="Model name"),
    version: str = PathParam(..., description="Model version"),
    stage: Literal["Staging", "Production", "Archived", "None"] = Query(..., description="Target stage")
):
    """
    ...
    try:
        ...
        return format_error_response(e, f"/api/v1/models/{model_name}/versions/{version}/transition")


@app.delete("/api/v1/models/{model_name}/versions/{version}", summary="Delete model version")
async def delete_model_version(
    ...
    try:
        ...
        return format_error_response(e, f"/api/v1/models/{model_name}/versions/{version}")
    ...
    try:
        ...
        return {
            ...
            "statistics": {
                ...
                "active_experiments": len([e for e in experiments if e.lifecycle_stage == "active"])
            },
            "model_registry": {
                "enabled": True,
                "backend": "PostgreSQL",
            ...
            "schema_validation": {
                "enabled": True,
                "experiments": list(schema.get('experiments', {}).keys()) if schema else [],
                "privacy_requirements": {
                    "min_recall": 0.95,
                    "max_false_negative_rate": 0.05,
            ...
            "artifact_organization": schema.get('global', {}).get('artifact_organization', {}) if schema else {},
            "best_practices": {
