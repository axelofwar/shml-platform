...
from fastapi import FastAPI, HTTPException, UploadFile, File, Form, Path as PathParam, Query, Depends, Header, BackgroundTasks, Request
...
# Prometheus Metrics
REQUEST_COUNT = Counter('mlflow_api_requests_total', 'Total API requests', ['method', 'endpoint', 'status'])
REQUEST_LATENCY = Histogram('mlflow_api_request_duration_seconds', 'Request latency', ['method', 'endpoint'])
...
RATE_LIMIT_HITS = Counter('mlflow_api_rate_limit_hits_total', 'Rate limit hits', ['user_tier'])
AUTH_FAILURES = Counter('mlflow_api_auth_failures_total', 'Authentication failures', ['method'])
...
async def get_current_user(
    ...
    raise HTTPException(status_code=401, detail="Authentication required. Provide OAuth token or API key.")
...
def validate_run_against_schema(
    ...
    if 'false_negative_rate' in metrics and metrics['false_negative_rate'] > max_fn_rate:
        warnings.append(
...
async def create_run(
    ...
    try:
        ...
        if not experiment:
            raise HTTPException(404, f"Experiment '{request.experiment_name}' not found")
        ...
        if warnings:
            ...
            response["message"] = "Run created with warnings. Review before production deployment."
        else:
...
async def upload_artifact_async(
    ...
    try:
        ...
        return {
            ...
            "compression_ratio": f"{(1 - compressed_size/original_size)*100:.1f}%" if compress and original_size > 0 else "none",
