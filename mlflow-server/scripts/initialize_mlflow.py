...
def create_experiment_if_not_exists(client, name, tags=None, description=None):
    ...
    try:
        ...
        if experiments:
            exp = experiments[0]
            logger.info(f"✓ Experiment '{name}' already exists (ID: {exp.experiment_id})")
...
def setup_model_registry(client):
    ...
    try:
        ...
        logger.info(f"✓ Model registry accessible ({len(registered_models)} models registered)")
        return True
    except Exception as e:
...
def enable_experimental_features(client):
    ...
    return {
        "traces": traces_exp_id,
        "evaluations": eval_exp_id
    }
