...
MODELS = [
    {
        ...
        "stage": "Archived",
        "aliases": ["baseline"],
        "metrics": {"map50": 0.88, "precision": 0.87, "recall": 0.80},
        "tags": {
            "version": "1.0.0",
...
def register_model_version(model_info, client):
    ...
    try:
        ...
        with mlflow.start_run(
            experiment_id=exp_id, run_name=f"model_registry_{model_info['name']}"
        ):
            # Log metrics
        ...
        # Set stage via alias (migrated from deprecated transition_model_version_stage)
