        ...
        if os.path.exists(artifact_path):
            for root, dirs, files in os.walk(artifact_path):
                for file in files:
                    ...
                    print(f"  Run: {run_id[:8]}... | {file_info['size_human']:>8} | {file}")
...
def plan_consolidation(datasets):
    ...
    wider_train_parts = [f for f in datasets.get('WIDER-train', []) if 'part' in f['filename']]
    wider_test_parts = [f for f in datasets.get('WIDER-test', []) if 'part' in f['filename']]
        ...
        if dry_run:
            ...
            try:
                with mlflow.start_run(run_name=item['name']) as run:
                    ...
                    print(f"    View: {TRACKING_URI}/#/experiments/{experiment_id}/runs/{run.info.run_id}")
...
def main():
    ...
    parser.add_argument('--report', type=str, default='consolidation_report.txt', help='Report output file')
