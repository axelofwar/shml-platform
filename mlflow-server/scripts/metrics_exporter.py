...
ARTIFACT_UPLOADED = Counter(
    "mlflow_artifact_uploaded_total", "Total artifacts uploaded"
)
ARTIFACT_COMPRESSED = Counter(
    "mlflow_artifact_compressed_total", "Total artifacts compressed"
)

# Performance metrics
RUN_DURATION = Histogram("mlflow_run_duration_seconds", "Run duration in seconds")
ARTIFACT_UPLOAD_SIZE = Histogram(
    "mlflow_artifact_upload_size_mb",
    "Artifact upload size in MB",
        ...
        try:
            ...
            for exp in experiments:
                runs = self.client.search_runs(
                    experiment_ids=[exp.experiment_id], max_results=10000
                )
            ...
            if self.artifact_root.exists():
                total_size = sum(
                    f.stat().st_size
                    for f in self.artifact_root.rglob("*")
                    if f.is_file()
                )
